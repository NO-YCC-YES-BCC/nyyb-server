 ...
 * 제품 마스터 검색 조건.
 *
 * ITEM_NAME은 "헤브블루살몬케어링센텔라크림"처럼 띄어쓰기가 없어서(전체 19.5만 건 중 공백 포함은 539건뿐)
 * 사용자가 입력한 "헤브블루 크림"을 통째로 매칭할 수 없다. 그래서 검색어를 공백으로 쪼갠 뒤
 * 모든 토큰이 이름 안에 들어 있는 제품을 찾는다. (AND 조건)
 *
 * 다만 LIKE '%...%'는 인덱스를 못 타서 전체 스캔이 된다. 브랜드명은 거의 항상 이름 맨 앞에 오므로
 * (예: '헤브블루' 40건 전부가 앞자리 시작) 첫 토큰만 접두 매칭으로 걸어 idx_item_name을 태우고
